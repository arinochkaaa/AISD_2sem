...
int MultiplicativeHash(int key, int size) {
	...
	hashValue -= int(hashValue); // Извлечение дробной части
	return int(size * hashValue);
}
...
int Object::searchInd(int key)
{
	...
	bool b = false;
	if (N != 0)
		for (int i = 0, j = HashFunction(key, size, 0); data[j] != NULL && i != size && !b; j = Next_hash(key, size, ++i))
			if (data[j] != DEL)
